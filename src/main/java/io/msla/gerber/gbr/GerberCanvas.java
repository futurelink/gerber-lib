...
public class GerberCanvas {
    private final static Logger log = Logger.getLogger(GerberCanvas.class.getName());
    ...
    public final void writeToGerber(Gerber gerber) throws GerberException {
        currentAperture = 0;
        currentInterpolation = null;
        currentQuadrantMode = Geometry.QuadrantMode.SINGLE;
        currentCoordinate = new Point(0, 0);
        ...
        if (apertures.isEmpty()) log.log(Level.WARNING, "No apertures defined");
        if (geometry.isEmpty()) log.log(Level.WARNING, "No geometry objects defined - canvas is empty");
        ...
        for (var g : geometry) {
            ...
            if (isFeaturedGeometry(g)) {
                ...
                if (g instanceof Line l) {
                    ...
                    throw new GerberException("Arcs modified by features are not supported yet");
                }
            }
